...
class EstadoDocumentoResource extends Resource
{
    ...
    public static function form(Schema $schema): Schema
    {
        return $schema->components([
            TextInput::make('nombre')->label('Nombre')->required()->maxLength(255)->rules(fn($record) => [
                Rule::unique('estado_documento', 'nombre')->ignore($record?->id)->where(fn($q) => $q->where('aso_id', session('aso_actual'))->whereNull('deleted_at')),
            ]),
        ...
        return $table
            ...
            ->toolbarActions([
                BulkAction::make('exportar')
                    ...
                    ->action(fn(Collection $records) => Excel::download(new EstadoDocumentoExport($records->pluck('id')), 'estado_documento.xlsx')),
