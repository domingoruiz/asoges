...
class TipoTransaccionResource extends Resource
{
    ...
    public static function form(Schema $schema): Schema
    {
        return $schema->components([
            TextInput::make('nombre')->label('Nombre')->required()->maxLength(255)->rules(function ($record) { return [ Rule::unique('tipo_transaccion','nombre')->ignore($record?->id)->where(fn($q)=>$q->where('aso_id', session('aso_actual'))->whereNull('deleted_at')), ]; }),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->toolbarActions([
                BulkAction::make('exportar')->label('Exportar seleccionados')->icon('heroicon-o-arrow-down-tray')->action(fn (Collection $records) => Excel::download(new TipoTransaccionExport($records->pluck('id')), 'tipo_transaccion.xlsx')),
