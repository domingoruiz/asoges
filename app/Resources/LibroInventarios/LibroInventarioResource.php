    ...
    public static function form(Schema $schema): Schema
    {
        return $schema->components([
            Section::make('Datos generales')
                ->schema([
                    TextInput::make('nombre')->label('Nombre')->required()->maxLength(255)->unique(table: 'inventario', column: 'nombre', ignoreRecord: true, modifyRuleUsing: fn($rule) => $rule->where('aso_id', session('aso_actual'))),
                    DatePicker::make('fecha_adquisicion')->label('Fecha de adquisición')->required()->displayFormat('d-m-Y')->format('Y-m-d')->maxDate(now())->native(false),
                    Select::make('categoria_id')->label('Categoría')->required()->searchable()->preload()->options(fn() => self::buildTreeOptions(CategoriaInventario::class)),
            ...
            Section::make('Valores')
                ->schema([
                    TextInput::make('cantidad')->label('Cantidad')->numeric()->required()->step('0.01')->minValue(0.01),
                    TextInput::make('valor')->label('Valor')->numeric()->required()->step('0.01')->minValue(0)->suffix('€'),
                ])
                ->columns(2),
            Section::make('Observaciones')
                ->schema([
                    Textarea::make('descripcion')->label('Observaciones')->rows(4),
                ]),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('categoria.nombre')->label('Categoría')->formatStateUsing(fn($state, $record) => $record->categoria?->ruta ?? '')->wrap()->sortable(),
                TextColumn::make('ubicacion.nombre')->label('Ubicación')->formatStateUsing(fn($state, $record) => $record->ubicacion?->ruta ?? '')->wrap()->sortable(),
                TextColumn::make('cantidad')->label('Cantidad')->sortable()->summarize([Sum::make()->label('Total cantidad')->formatStateUsing(fn($state) => number_format($state, 2, ',', '.'))]),
                TextColumn::make('valor')->label('Valor')->sortable()->formatStateUsing(fn($state) => number_format($state, 2, ',', '.') . ' €')->summarize([Sum::make()->label('Total')->formatStateUsing(fn($state) => number_format($state, 2, ',', '.') . ' €')]),
            ])
            ->filters([
                ...
                SelectFilter::make('categoria_id')->label('Categoría')->options(fn() => self::buildTreeOptions(CategoriaInventario::class)),
            ...
            ->toolbarActions([
                ...
                BulkAction::make('exportar')->label('Exportar seleccionados')->icon('heroicon-o-arrow-down-tray')->action(fn(Collection $records) => Excel::download(new LibroInventarioExport($records->pluck('id')), 'libro_inventario.xlsx')),
            ])
