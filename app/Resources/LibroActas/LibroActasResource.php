...
class LibroActasResource extends Resource
{
        ...
        return $schema->components([
            Section::make('Datos del acta')
                ->schema([
                    ...
                    DatePicker::make('fecha')->label('Fecha')->required()->maxDate(now()),
                    TimePicker::make('hora_inicio')->label('Hora inicio')->required()->seconds(false),
                    TimePicker::make('hora_fin')->label('Hora fin')->required()->seconds(false)->rule('after_or_equal:hora_inicio'),
                    TextInput::make('lugar_reunion')->label('Lugar de reunión')->required()->maxLength(255),
                ])
                ->columns(2),
            ...
            Section::make('Contenido del acta')
                ->schema([
                    RichEditor::make('contenido_acta')->label('Contenido')->required()->columnSpanFull(),
                ]),
            Section::make('Aprobación')
                ->schema([
                    Toggle::make('aprobada')->label('Aprobada')->inline(false)->reactive(),
                    DateTimePicker::make('fecha_aprobacion')->label('Fecha de aprobación')->seconds(false)->visible(fn(callable $get) => (bool) $get('aprobada'))->required(fn(callable $get) => (bool) $get('aprobada'))->maxDate(now()),
                ])
                ->columns(2),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->toolbarActions([
                BulkAction::make('exportar')->label('Exportar seleccionados')->icon('heroicon-o-arrow-down-tray')->action(fn(Collection $records) => Excel::download(new LibroActasExport($records->pluck('id')), 'libro_actas.xlsx')),
