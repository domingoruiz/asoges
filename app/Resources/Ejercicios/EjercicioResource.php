    ...
    public static function form(Schema $schema): Schema
    {
        return $schema->components([
            ...
            TextInput::make('nombre')
                ...
                ->required()
                ->maxLength(255)
                ->unique(table: 'ejercicio', column: 'nombre', ignoreRecord: true, modifyRuleUsing: fn(Unique $rule) => $rule->where('aso_id', session('aso_actual'))),
            DatePicker::make('fch_inicio')->label('Fecha inicio')->required()->default(fn() => now()->startOfYear()),
            DatePicker::make('fch_fin')->label('Fecha fin')->required()->rule('after_or_equal:fch_inicio')->default(fn() => now()->endOfYear()),
        ]);
    }
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('fch_inicio')->label('Inicio')->date('d/m/Y')->sortable(),
                TextColumn::make('fch_fin')->label('Fin')->date('d/m/Y')->sortable(),
            ])
            ...
            ->toolbarActions([
                BulkAction::make('exportar')
                    ...
                    ->action(fn(Collection $records) => Excel::download(new EjerciciosExport($records->pluck('id')), 'ejercicios.xlsx')),
